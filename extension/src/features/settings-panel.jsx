...
async function initSettingsPanel(activate = true) {
    ...
    const settingsPanelEl = (
        <div id="update-content-modal-modal-content" class="modal-content">
            <div class="modal-header">
                <a class="close" onClick={handleClose}><i class="icon-cancel"></i></a>
                <svg xmlns="http://www.w3.org/2000/svg" width="24px" height="24px" style={{float: "left", marginRight: 7}} viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="lucide lucide-rocket"><path d="M4.5 16.5c-1.5 1.26-2 5-2 5s3.74-.5 5-2c.71-.84.7-2.13-.09-2.91a2.18 2.18 0 0 0-2.91-.09z"/><path d="m12 15-3-3a22 22 0 0 1 2-3.95A12.88 12.88 0 0 1 22 2c0 2.72-.78 7.5-6 11a22.35 22.35 0 0 1-4 2z"/><path d="M9 12H4s.55-3.03 2-4c1.62-1.08 5 0 5 0"/><path d="M12 15v5s3.03-.55 4-2c1.08-1.62 0-5 0-5"/></svg>
                <h6 class="modal-title" data-testid="modal-title">iRefined</h6>
            </div>
            <div id="alert-banner-alert-1739463611065" class="alert alert-banner m-b-0 alert-warning alert-dismissible text-overflow" role="alert">
                <div class="">
                    <a class="close"><i class="icon-cancel"></i></a>
                    <span><i class="icon-caution m-r-1"></i><strong></strong></span>
                </div>
            </div>
            <div class="modal-body has-dynamic-height">
                <div id="modal-children" class="height-limiter" style={{maxHeight: 1244}}>
                    <div id="modal-children-container" style={{position: "relative"}}>
                        <div>
                            <div class="row">
                                <div class="col-xs-12">
                                    <h1 class="m-b-1"><strong>Settings</strong></h1>
                                    <label htmlFor="" class="iref-setting">
                                        ...
                                        <input type="checkbox" name="share-test-session" checked={settings["share-test-session"]} onChange={handleChange} />
                                    </label>
                                    <label htmlFor="" class="iref-setting">
                                        ...
                                        <input type="checkbox" name="share-hosted-session" checked={settings["share-hosted-session"]} onChange={handleChange} />
                                    </label>
                                    <label htmlFor="" class="iref-setting">
                                        ...
                                        Queue registrations
                                        <input type="checkbox" name="auto-register" checked={settings["auto-register"]} onChange={handleChange} />
                                    </label>
                                    <label htmlFor="" class="iref-setting">
                                        <i class="icon-information text-info" title="Green join button will display session type. Official race sessions get an extra border. Doesn't work well with official sessions that don't go official (low attendance)."></i>
                                        Join button displays session type
                                        <input type="checkbox" name="better-join-button" checked={settings["better-join-button"]} onChange={handleChange} />
                                    </label>
                                    <label htmlFor="" class="iref-setting">
                                        ...
                                        <select name="auto-join-type" onChange={handleChange}>
                                            <option value="race" selected={settings["auto-join-type"] == "race"}>race</option>
                                            <option value="all" selected={settings["auto-join-type"] == "all"}>all</option>
                                        </select>
                                        sessions
                                        <input type="checkbox" name="auto-join" checked={settings["auto-join"]} onChange={handleChange} />
                                    </label>
                                    <label htmlFor="" class="iref-setting">
                                        ...
                                        Auto forfeit after
                                        <input type="number" name="auto-forfeit-m" value={settings["auto-forfeit-m"] || 13} onChange={handleChange} />
                                        minutes
                                        <input type="checkbox" name="auto-forfeit" checked={settings["auto-forfeit"]} onChange={handleChange} />  
                                    </label>
                                    <label htmlFor="" class="iref-setting">
                                        <i class="icon-information text-info" title="Don't show any notifications at the top of the screen."></i>
                                        No results notifications
                                        <input type="checkbox" name="no-toasts" checked={settings["no-toasts"]} onChange={handleChange} />
                                    </label>
                                    <label htmlFor="" class="iref-setting">
                                        ...
                                        <input type="number" name="toast-timeout-s" value={settings["toast-timeout-s"] || 5} onChange={handleChange} />
                                        seconds
                                        <input type="checkbox" name="auto-close-toasts" checked={settings["auto-close-toasts"]} onChange={handleChange} />  
                                    </label>
                                    <label htmlFor="" class="iref-setting">
                                        <i class="icon-information text-info" title="Hide the left and right sidebars for a cleaner UI with more space."></i>
                                        Hide sidebars
                                        <input type="checkbox" name="no-sidebars" checked={settings["no-sidebars"]} onChange={handleChange} />
                                    </label>
                                    <label htmlFor="" class="iref-setting">
                                        ...
                                        Collapse menu
                                        <input type="checkbox" name="collapse-menu" checked={settings["collapse-menu"]} onChange={handleChange} />
                                    </label>
                                    <label htmlFor="" class="iref-setting">
                                        <i class="icon-information text-info" title="Helpful to figure out why something happened."></i>
                                        Show iRefined log messages
                                        <input type="checkbox" name="logger" checked={settings["logger"]} onChange={handleChange} />
                                    </label>
                                </div>
                            </div>
                        </div>
            ...
            <div class="modal-footer">
                <div>
                    ...
                    <div class="pull-xs-right">
                        <span class="m-l-h">
                            <button type="button" class="btn btn-success" aria-label="button" tabindex="0" onClick={handleSave}>Save</button>
                        </span>
                    </div>
                </div>
            </div>
    ...
    const menuButtonEl = (
        <button type="button" className="iref-toolbar-btn" aria-label="iRefined" tabindex="0" onClick={handleClick}>
